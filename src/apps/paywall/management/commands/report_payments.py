...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument('--id_plan', nargs='?', type=str)
    ...
    def get_resumen(self, siebel_entecode, delivery, obj):
    ...
    def handle(self, *args, **options):
        ...
        with open('/tmp/usuarios_payments_paywall.csv', 'a') as csvFile:
            ...
            writer.writerow([
                ...
                smart_str(u"recurrencia envio"),
                smart_str(u"recurrencia response"),
                smart_str(u"valor de renovacion")
            ])
            if options.get('id_plan'):
                users = Operation.objects.filter(
                    plan__arc_pricecode=options.get('id_plan')
                )
            elif options.get('lista_payments'):
                users = Operation.objects.filter(
            ...
            for obj in users.iterator():
                ...
                resumen = self.get_resumen(siebel_entecode, delivery, obj)
                transaction_id = self.get_transaction_id(obj)
                if obj.recurrencia_request:
                    renovation_send = obj.recurrencia_request
                    dict_renovation = json.loads(renovation_send.replace("'", "\""))
                    renovation_value = dict_renovation.get('renovacion')
