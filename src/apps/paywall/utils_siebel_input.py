            ...
            if self.operation.payment.pa_origin == 'RECURRENCE':
                ...
                if self.operation.siebel_delivery:
                    ...
                    try:
                        ...
                        if not id_delivery:
                            print('no tiene delivery: ' + str(self.subscription.arc_id))
                            with push_scope() as scope:
                                scope.set_tag("id_subscription", self.subscription.arc_id)
                                scope.level = 'warning'
                                capture_event(
                                    {
                                        'message': 'delivery_empty',
                                        'extra': {
                                            'id_subscription': self.subscription.arc_id,
                                            'obj_operation': obj_operation
                                        }
                                    }
                                )
                    except Exception as e:
                        ...
                        with push_scope() as scope:
                            ...
                            # scope.user = {'id_subscription': self.subscription.arc_id,
                            #              'order_number': self.operation.payment.arc_order}
                            scope.level = 'warning'
                            capture_event(
                                {
                                    'message': 'delivery_not_found_error',
                                    'extra': {
                                        'id_subscription': self.subscription.arc_id,
                                        'error': e
                                    }
                ...
                if ente_code and id_delivery:
                    ...
                    if not id_delivery:
                        print('delivery_not_found: para suscripcion: ' + str(self.subscription.arc_id))
                        with push_scope() as scope:
                            scope.set_tag("id_subscription", self.subscription.arc_id)
                            scope.set_tag("order_number", self.operation.payment.arc_order)
                            scope.level = 'warning'
                            capture_event(
                                {
                                    'message': 'delivery_or_not_found',
                                    'extra': {
                                        'id_subscription': self.subscription.arc_id,
                                        'id_delivery': id_delivery,
                                        'type_id_delivery': type(id_delivery)
                                    }
                                }
                            )
                    if not ente_code:
                        print('entecode_not_found: para suscripcion: ' + str(self.subscription.arc_id))
                        with push_scope() as scope:
                            scope.set_tag("id_subscription", self.subscription.arc_id)
                            scope.set_tag("order_number", self.operation.payment.arc_order)
                            scope.level = 'warning'
                            capture_event(
                                {
                                    'message': 'entecode_not_found',
                                    'extra': {
                                        'id_subscription': self.subscription.arc_id,
                                        'ente_code': self.profile.siebel_entecode,
                                        'profile': self.profile,
                                        'type_ente': type(self.profile.siebel_entecode)
                                    }
                                }
                            )
            else:
