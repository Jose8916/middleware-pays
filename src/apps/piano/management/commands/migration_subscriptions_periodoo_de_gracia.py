...
class Command(BaseCommand):
    ...
    def write_migration(self, list_subs, list_terms, list_tokens, list_no_cumple_condicion, brand, writer_no_cumple_condicion, writer, flag):
        for row in list_subs:
            ...
            if data_subscription:
                if data_subscription.get('status') != Subscription.ARC_STATE_TERMINATED and row.get(
                        'initialTransaction') == 'True' and data_subscription.get('status') == Subscription.ARC_STATE_SUSPENDED:
                    ...
                    print(data_subscription.get('status'))
                    if flag == 1:
    ...
    def handle(self, *args, **options):
        ...
        """
        path = '/home/milei/Documentos/subscription_migration_gracia/'
        list_tokens = []
        list_terms = []
        list_subscriptions = []
        count = 0
        ...
        if options.get('site'):
            ...
            try:
                with open(path + rama + '_gestion/list_terms_gestion.csv') as csvfileTerms:
            ...
            name_file_migration_subscriptions = '{path}{rama}/{brand}_subs_payu_elizabeth.csv'.format(
            ...
            with open(name_file_migration_subscriptions, 'a', encoding="utf-8") as csvFilewrite:
                ...
                with open(path + rama + '/no_cumple_condicion.csv', 'a', encoding="utf-8") as csvFile2:
                    ...
                    print(path + rama + '/backup_' + rama + '.csv')
