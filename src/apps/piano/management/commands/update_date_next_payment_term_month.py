...
class Command(BaseCommand):
    ...
    def update_next_billing_date(self, term_id, date_to_update):
        ...
        if term_id in \
            ...
            date_time_obj = date_time_obj + relativedelta(months=3)
            return date_time_obj.strftime("%m/%d/%Y %H:%M")
        elif term_id in [
            'TM7G2TN8N4D2',
            'TMSUQW75VXLA',
            ...
            date_time_obj = date_time_obj + relativedelta(months=6)
            return date_time_obj.strftime("%m/%d/%Y %H:%M")
        elif term_id in [
            'TMYRU1VPC9UJ',
            'TM3JMVV96O9Z',
            ...
            'TMB27UV9E63S',
            'TMX415TO8OET']:  # anual
            date_time_obj = date_time_obj + relativedelta(years=1)
            return date_time_obj.strftime("%m/%d/%Y %H:%M")
        else:
            return None
    ...
    def handle(self, *args, **options):
        ...
        name_file_migration_subscriptions = '/home/milei/Escritorio/reporte_erick/primer_formato1.csv'.format(
        ...
        with open(name_file_migration_subscriptions, 'a', encoding="utf-8") as csvFilewrite:
            ...
            with open('/home/milei/Escritorio/reporte_erick/primer_formato.csv') as csvfile:
                reader = csv.DictReader(csvfile)
                for row in reader:
                    if row.get('next_billing_date_arc') and row.get('next_billing_date_arc') != '#N/D':
